# Meridian RAG Assistant & Evaluation Gate (OWNER)
...
* **Evaluation Framework**: Custom offline subset benchmarking & Ragas-style metrics with Langfuse tracing
...
├── golden_set.json              # 61-case evaluation golden set (including production drift additions)
...
* **Inference Costs:** Based on Groq high-speed model pricing (≈ $0.05 / M input tokens, ≈ $0.08 / M output tokens). Assuming 500 input tokens and 150 output tokens per request across 50 requests (25,000 input tokens, 7,500 output tokens), the total estimated cost is $0.0018 (less than a fraction of a cent).

* **Langfuse Evaluation Unit Model:**

* Baseline trace footprint of ~9 units per question before scoring (~531 units for a full run).

* Incorporating 5x judge repetitions for stability brings a comprehensive evaluation run to approximately 2,800–3,000 units.

* Under a 50,000-unit monthly cap, the system supports roughly 16–18 full evaluation runs per month.
...
**D. Production Monitoring Signals, Drift & Re-sampling**
* **Re-sampling Cadence:** Bi-weekly (every 2 weeks) sampling of live production logs (`production_runs.json`) to catch emerging category drift and prevent evaluation suite rot.
...
* **First Response Action:**
    ...
    * **Check Database Health:** Navigate to the database hosting dashboard to inspect active connections, CPU utilization, and storage limits.
    * **Verify Credentials & Network:** Confirm environment variables (DATABASE_URL) and security group rules are properly configured.
    * **Mitigate & Restart:** Scale connection limits if saturated, restart the FastAPI container service, and monitor the health endpoint until error rates drop to zero.
